      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-3rem)] w-full max-w-6xl flex-col gap-6">
        <header className="relative overflow-hidden rounded-4xl border border-white/10 bg-zinc-900/80 p-5 shadow-[0_24px_80px_-30px_rgba(0,0,0,0.5)] backdrop-blur-xl sm:p-6">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <nav className="flex flex-wrap gap-2" aria-label="Primary">
              ...
              <NavLink to="/2026-rankings" className={linkClassName}>
                2026 Rankings
              </NavLink>
            </nav>
          </div>
        ...
        <main className="grid flex-1 place-items-center py-2 sm:py-4">
          <Routes>
            ...
            <Route path="/2026-rankings" element={<RankingsPage />} />
      ...
      <div className="relative flex flex-col gap-8 sm:gap-10">
        ...
        <div className="max-w-2xl">
          ...
          <p className="mt-4 max-w-prose text-base leading-7 text-slate-300 sm:text-lg">
            UniMetric lets you cut through the noise — pick the ranking factors that matter to you and see which universities rise to the top.
          </p>
          <div className="mt-7">
            <NavLink
              to="/2026-rankings"
            ...
            >
              Explore 2026 Rankings
              <svg className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" /></svg>
            </NavLink>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 border-t border-white/8 pt-7 sm:gap-6 sm:pt-8">
          <div>
            <p className="text-2xl font-bold tabular-nums text-white sm:text-3xl">1,501</p>
            <p className="mt-1 text-xs text-slate-300 sm:text-sm">Universities ranked</p>
          </div>
          <div>
            <p className="text-2xl font-bold tabular-nums text-white sm:text-3xl">10</p>
            <p className="mt-1 text-xs text-slate-300 sm:text-sm">QS ranking factors</p>
          </div>
          <div>
            <p className="text-2xl font-bold tabular-nums text-cyan-300 sm:text-3xl">2026</p>
            <p className="mt-1 text-xs text-slate-300 sm:text-sm">Latest QS data</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {[
            { title: "Custom weights", body: "Choose which factors matter most to you — reputation, research, international mix — and rerank instantly." },
            { title: "Instant search", body: "Filter 1,500+ institutions by name in real time, no page reloads." },
            { title: "Score breakdown", body: "Tap any ranking card to explore each university's score across all 10 QS factors." },
          ].map(({ title, body }) => (
            <div key={title} className="rounded-2xl border border-white/8 bg-white/8 p-4 sm:p-5">
              <p className="text-sm font-semibold text-white">{title}</p>
              <p className="mt-1.5 text-xs leading-5 text-slate-300 sm:text-sm sm:leading-6">{body}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="relative w-full max-w-6xl overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/80 p-5 shadow-[0_28px_90px_-32px_rgba(0,0,0,0.6)] backdrop-blur-xl sm:rounded-[2.25rem] sm:p-8">
      ...
      <div className="relative flex flex-col gap-5 sm:gap-6">
        ...
        <div className="flex flex-col gap-3">
          <p className="text-center text-sm font-medium text-slate-300 sm:text-left">Select what factors you care about:</p>
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-baseline sm:gap-x-6 sm:gap-y-3">
            {([
              {
                group: "Reputation",
                factors: [
                  { key: "arScore", label: "Academic Reputation", audience: "ug" },
                  { key: "erScore", label: "Employer Reputation", audience: "ug" },
                ],
              },
              {
                group: "Teaching & Outcomes",
                factors: [
                  { key: "fsrScore", label: "Faculty/Student Ratio", audience: "ug" },
                  { key: "eoScore", label: "Employment Outcomes", audience: "ug" },
                  { key: "susScore", label: "Sustainability", audience: "ug" },
                ],
              },
              {
                group: "Research",
                factors: [
                  { key: "cpfScore", label: "Citations per Faculty", audience: "pg" },
                  { key: "irnScore", label: "Intl. Research Network", audience: "pg" },
                ],
              },
              {
                group: "International",
                factors: [
                  { key: "ifrScore", label: "Intl. Faculty Ratio", audience: "pg" },
                  { key: "isrScore", label: "Intl. Student Ratio", audience: "pg" },
                  { key: "isdScore", label: "Intl. Student Diversity", audience: "pg" },
                ],
              },
            ] as { group: string; factors: { key: FactorKey; label: string; audience: "ug" | "pg" }[] }[]).map(({ group, factors: groupFactors }) => (
              <div key={group} className="flex flex-col items-center gap-1.5 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-2 sm:gap-y-1.5">
                <span className="text-xs font-semibold uppercase tracking-widest text-slate-400">{group}</span>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="flex flex-1 flex-col gap-2">
            <span className="text-sm font-medium text-slate-200">Search rankings</span>
            <input
              type="search"
          ...
          <label className="flex flex-col gap-2 sm:w-52">
            ...
            <div className="relative">
              ...
              <svg className="pointer-events-none absolute right-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" aria-hidden="true">
